...
def generate_synthetic_data(
    start_date: str = "2023-01-01", periods: int = 240, freq: str = "H", seed: int = 42
) -> pd.DataFrame:
    """Generate synthetic ERCOT-style time series data."""
    np.random.seed(seed)
    date_range = pd.date_range(start=start_date, periods=periods, freq=freq)
    load = (
        np.sin(np.linspace(0, 12 * np.pi, periods)) * 20
        + 100
...
def split_data(
    df: pd.DataFrame, hold_out_hours: int = 24
...
def fit_statsforecast(
    train: pd.DataFrame, season_length: int = 24, freq: str = "h", n_jobs: int = -1
) -> StatsForecast:
    """Fit StatsForecast model."""
    sf = StatsForecast(
        models=[AutoARIMA(season_length=season_length)], freq=freq, n_jobs=n_jobs
    )
    sf.fit(train)
    return sf
